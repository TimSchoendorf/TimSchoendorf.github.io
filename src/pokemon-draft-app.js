  ...
  app.innerHTML = `
    <div class="draft-shell">
      ...
      <main class="column center">
        ${state.phase === 'draft' ? renderDraftStage() : renderBattleStage()}
      </main>
...
function renderCombatant(mon, label) {
  ...
  return `
    <div class="combat-card" style="background:${typeGradient(mon.types || ['Normal'])}">
      ...
      <div class="tiny">${mon.status || mon.item || ''}</div>
    </div>
  `;
}
...
function updatePlayerStateFromRequest(request) {
  state.playerTeamState = request.side.pokemon.map((mon, index) => {
    const drafted = state.playerDraft[index];
    return {
      ...
      active: mon.active,
      item: mon.item,
      status: mon.condition.endsWith(' fnt') ? 'fainted' : '',
    };
  });
...
async function startBattle() {
  ...
  const playerTeam = state.playerDraft.map((species) => generateSet(species, Dex));
  const opponentTeam = state.opponentDraft.map((species) => generateSet(species, Dex));
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    *{box-sizing:border-box}body{margin:0;min-height:100vh;font-family:"Segoe UI",sans-serif;background:radial-gradient(circle at top left,rgba(255,135,94,.18),transparent 25%),radial-gradient(circle at bottom right,rgba(255,204,103,.18),transparent 24%),linear-gradient(180deg,#14070d,#24111d 52%,#0f111f);color:var(--cream);padding:24px}
    ...
    .hero-panel{padding:18px;display:flex;justify-content:space-between;gap:16px;align-items:end}.draft-message{padding:14px 16px;min-height:64px;max-width:360px}.draft-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:16px}.draft-card{padding:18px;text-align:left;color:#1a1012;cursor:pointer;transition:transform .16s ease}.draft-card:hover{transform:translateY(-3px)}.draft-num{font-size:.82rem;opacity:.7}.draft-card h3{margin:8px 0 10px;font-size:1.6rem}.stat-strip{display:flex;flex-wrap:wrap;gap:10px;font-size:.84rem;margin-top:10px}
    .battle-field{display:grid;gap:14px;padding:18px;border-radius:28px;background:linear-gradient(180deg,rgba(255,255,255,.03),rgba(255,255,255,.06))}.combatant.top{justify-self:end}.combatant.bottom{justify-self:start}.combat-card{min-width:260px;padding:18px;color:#1a1012}.hp-bar{height:12px;background:rgba(0,0,0,.14);border-radius:999px;overflow:hidden;margin-top:10px}.hp-fill{height:100%;background:linear-gradient(90deg,#5be18c,#d6ffb8)}
    .arena-center{min-height:180px;border-radius:26px;background:radial-gradient(circle at center,rgba(255,204,103,.18),transparent 36%),rgba(255,255,255,.03);display:grid;place-items:center;position:relative;overflow:hidden}.arena-ring{width:220px;height:220px;border-radius:50%;border:2px solid rgba(255,255,255,.12)}.arena-log{position:absolute;inset:auto 18px 18px 18px;display:grid;gap:6px;font-size:.88rem;color:#ffe6da}
    ...
    @media (max-width:1120px){.draft-shell{grid-template-columns:1fr}.draft-grid,.choice-grid{grid-template-columns:1fr}}
  `;
