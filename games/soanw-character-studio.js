...
function sanitizePageText(page) {
  ...
  return repeatedAt > 120 ? cleaned.slice(0, repeatedAt).trim() : cleaned.trim();
}
  ...
  return `
    ${panel('Identity', `
      <div class="form-grid two">
        ...
        <label><span>Species</span><select data-field="profile.species">${SPECIES.map((item) => `<option ${item === c.profile.species ? 'selected' : ''}>${item}</option>`).join('')}</select></label>
        ...
        <label><span>Class</span><select data-field="profile.className">${CLASSES.map((item) => `<option ${item === c.profile.className ? 'selected' : ''}>${item}</option>`).join('')}</select></label>
        <label><span>Subclass</span><select data-field="profile.subclass">${(SUBCLASS_MAP[c.profile.className] || []).map((item) => `<option ${item === c.profile.subclass ? 'selected' : ''}>${item}</option>`).join('')}</select></label>
      </div>
    `, 'Stammdaten und Grundentscheidungen')}
    ${panel('Abilities', `
      ...
      <div class="summary-row">
        <div class="summary-chip">Proficiency +${proficiencyBonus()}</div>
        <div class="summary-chip">Initiative ${mod(c.abilities.dex) + Number(c.combat.initiativeBonus || 0) >= 0 ? '+' : ''}${mod(c.abilities.dex) + Number(c.combat.initiativeBonus || 0)}</div>
        <div class="summary-chip">Passive Perception ${10 + mod(c.abilities.wis)}</div>
      </div>
    `, 'D&D-artige Kernwerte, auf SoaNW zugeschnitten')}
    ${panel('Combat', `
    ...
    `, 'HP, Vitality, AC, Speed und Inventarlast')}
    ${panel('Proficiencies', `
      <div class="form-grid two">
        <label class="full"><span>Skill Proficiencies</span>${renderOptionList(SKILLS, c.proficiencies.skills, 'proficiencies.skills')}</label>
    ...
    `)}
    ${panel('Build', `
      <label class="full"><span>Feats</span>${renderOptionList(FEATS, c.build.feats, 'build.feats')}</label>
      <div class="form-grid two">
        ...
        <label><span>History / Hook</span><textarea data-field="build.background">${c.build.background}</textarea></label>
      </div>
    `)}
    ${panel('Loadout', `
      <div class="form-grid two">
        ...
        <label><span>Vehicle</span><textarea data-field="loadout.vehicle">${c.loadout.vehicle}</textarea></label>
        <label><span>Money / Wealth</span><textarea data-field="loadout.money">${c.loadout.money}</textarea></label>
      </div>
    `)}
    ${panel('Magic & Maneuvers', `
      <label><span>Arias</span>${renderOptionList(spells.arias, c.magic.arias, 'magic.arias')}</label>
      <label><span>Divine Magic</span>${renderOptionList(spells.divine, c.magic.divine, 'magic.divine')}</label>
      <label><span>Elemental Magic</span>${renderOptionList(spells.elemental, c.magic.elemental, 'magic.elemental')}</label>
      <label><span>Wild Magic</span>${renderOptionList(spells.wild, c.magic.wild, 'magic.wild')}</label>
      <label><span>Witchcraft</span>${renderOptionList(spells.witchcraft, c.magic.witchcraft, 'magic.witchcraft')}</label>
      <label><span>Maneuvers</span>${renderOptionList(spells.maneuvers, c.magic.maneuvers, 'magic.maneuvers')}</label>
      <label><span>Magic Notes</span><textarea data-field="magic.notes">${c.magic.notes}</textarea></label>
    `, 'Aus den Handbook-Kapiteln vorgeladene Namenslisten')}
    ${panel('Character Notes', `
      <div class="form-grid two">
...
function handleFieldInput(event) {
  ...
  if (path === 'profile.className') {
    state.character.profile.subclass = SUBCLASS_MAP[input.value]?.[0] || '';
  }
  saveState();
  render();
}
...
function bindEvents() {
  document.querySelectorAll('[data-field]').forEach((input) => input.addEventListener('input', handleFieldInput));
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    *{box-sizing:border-box}body{margin:0;min-height:100vh;font-family:"Segoe UI",system-ui,sans-serif;background:radial-gradient(circle at top left,rgba(127,209,255,.16),transparent 24%),radial-gradient(circle at bottom right,rgba(240,195,108,.14),transparent 26%),linear-gradient(180deg,#100f18,#1b1420 52%,#120f18);color:var(--text);padding:24px}button,input,select,textarea{font:inherit}
    .studio-shell{width:min(1480px,100%);margin:0 auto;display:grid;grid-template-columns:320px minmax(0,1fr);gap:20px}.left-rail{display:grid}.summary-card,.panel,.hero,.tab-btn,.pill,.filter-btn,.page-btn,.primary-btn,.upload-btn{border:1px solid var(--line);border-radius:24px;background:rgba(25,22,34,.82);backdrop-filter:blur(14px)}.summary-card.sticky{position:sticky;top:24px;padding:20px}.back-link{text-decoration:none;display:inline-flex;margin-bottom:14px;padding:10px 14px;border-radius:999px;background:rgba(255,255,255,.06);color:var(--text)}h1,h2{margin:0}.muted{color:var(--muted)}.eyebrow{text-transform:uppercase;letter-spacing:.16em;font-size:.78rem;color:var(--accent)}
    ...
    .main-column{display:grid;gap:18px}.hero{padding:18px;display:flex;justify-content:space-between;gap:18px;align-items:end}.tab-row{display:flex;gap:10px;flex-wrap:wrap}.tab-btn{padding:12px 16px;color:var(--text);cursor:pointer}.tab-btn.active,.primary-btn,.upload-btn{background:linear-gradient(135deg,#f0c36c,#ffdd96);color:var(--ink)}
    ...
    .pill-grid{display:flex;flex-wrap:wrap;gap:8px}.pill,.filter-btn,.page-btn{padding:10px 12px;color:var(--text);cursor:pointer}.pill.active,.filter-btn.active,.page-btn.active{background:linear-gradient(135deg,#7fd1ff,#b4e7ff);color:var(--ink)}
    ...
    @media (max-width:1180px){.studio-shell,.compendium-shell,.form-grid.two,.form-grid.three,.ability-grid{grid-template-columns:1fr}.left-rail{order:2}.main-column{order:1}.summary-card.sticky{position:static}.hero{flex-direction:column;align-items:flex-start}}
